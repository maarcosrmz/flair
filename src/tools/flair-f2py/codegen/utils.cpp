...
str_t render(str_t tpl, std::initializer_list<std::pair<str_t, str_t>> subs) {
  for (auto const &[key, val] : subs) {
    ...
    for (size_t pos = tpl.find(hole); pos != str_t::npos; pos = tpl.find(hole, pos + val.size()))
...
str_t string_pool_t::intern(str_t const &lit) {
  if (auto it = seen.find(lit); it != seen.end()) return it->second;
...
str_t string_pool_t::decls() const {
  ...
  for (auto const &[var, lit] : entries)
    out += fmt::format("    character(kind=c_char, len={}), target, save :: {} = \"{}\"//c_null_char\n",
                       lit.size() + 1, var, lit);
  return out;
}
...
// Python class name: folded Fortran name with an uppercase initial (PEP 8), e.g. "point" -> "Point".
...
str_t struct_name(semantics::Symbol const &s) { return "py_" + tname(s) + "_object_t"; }
