...
str_t to_py(Fortran::semantics::DeclTypeSpec const &t, str_t const &expr) {
  auto cat = flu::category(t);
  if (cat) switch (*cat) {
    case TypeCategory::Real:    return "PyFloat_FromDouble(real(" + expr + ", c_double))";
    case TypeCategory::Integer: return "PyLong_FromLongLong(int(" + expr + ", c_long_long))";
    default: break;
    }
  return "c_null_ptr";
}
...
str_t from_py(Fortran::semantics::DeclTypeSpec const &t, str_t const &obj) {
  ...
  if (cat) switch (*cat) {
    case TypeCategory::Real:
      return "real(PyFloat_AsDouble(" + obj + "), " + std::to_string(flu::kind_of(t)) + ")";
    case TypeCategory::Integer:
      return "int(PyLong_AsLongLong(" + obj + "), " + std::to_string(flu::kind_of(t)) + ")";
    default: break;
    }
  return "0";
}
...
str_t ftype(Fortran::semantics::DeclTypeSpec const &t) {
  auto cat = flu::category(t);
  if (cat) switch (*cat) {
    case TypeCategory::Real:    return "real(" + std::to_string(flu::kind_of(t)) + ")";
    case TypeCategory::Integer: return "integer(" + std::to_string(flu::kind_of(t)) + ")";
    default: break;
    }
  return "";
}
