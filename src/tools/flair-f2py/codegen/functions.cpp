...
bool parse_args(std::vector<semantics::Symbol *> const &dummies, module_info_t const &m,
  ...
  for (semantics::Symbol *d : dummies) {
    ...
    if (str_t const dn = flu::derived_name(*t); !dn.empty()) {
      ...
      if (wt == nullptr) return false; // a type we don't wrap
...
str_t build_result(semantics::DeclTypeSpec const *rt, str_t const &call_expr) {
  if (rt != nullptr) return fmt::format("        r = {}\n", to_py(*rt, call_expr));
  str_t s;
  s += fmt::format("        call {}\n", call_expr);
...
std::vector<semantics::Symbol *> drop_self(std::vector<semantics::Symbol *> const &dummies) {
...
str_t gen_module_function(semantics::Symbol const &fn, module_info_t const &m,
  ...
  if (!fn.has<semantics::SubprogramDetails>()) return "";
  ...
  if (!parse_args(sub.dummyArgs(), m, "r = c_null_ptr", decls, fetch, call_args)) return "";

  semantics::DeclTypeSpec const *rt = sub.isFunction() ? sub.result().GetType() : nullptr;
  if (sub.isFunction() && (rt == nullptr || !intrinsic_supported(*rt))) return "";
